...
if($_FILES["user_picture"]['name'] != "") {
  ...
  if($check !== false) {
    ...
    header("Location: /fyp/edit-fyp_coordinator.php?fyp_coordinator_id=".$_POST['fyp_coordinator_id']);
  }
}
...
if (move_uploaded_file($_FILES["user_picture"]["tmp_name"], $target_dir.$unixTime.'.'.$imageFileType)) {
    $query = "update users set image = '".$unixTime.'.'.$imageFileType."' where id = '".$_POST['fyp_coordinator_id']."'";
    if(!$conn->query($query)){
